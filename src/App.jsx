...
function App() {
  ...
  const [parts, setParts] = useState(['', '', '']) // 3칸으로 축소
      ...
      {selectedCompany && (
        <div style={{ background: 'white', padding: '12px', borderRadius: '8px', boxShadow: '0 2px 6px rgba(0,0,0,0.08)' }}>
          ...
          <form onSubmit={handleSaveReport} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            {/* 5. 요청내용 */}
            <div>
              <label style={{ fontSize: '12px', fontWeight: 'bold' }}>요청내용</label>
              <textarea rows="3" value={requestContent} onChange={(e) => setRequestContent(e.target.value)} style={inputStyle} />
            </div>
